...
## How come floating point values are elements of `int` intervals?
...
## Why use semicolons in the string representation?

I'm using semicolons because the conventional representation of intervals (e.g. $(a, b)$ for open and $[c, d]$ for closed intervals) makes them look like tuples and lists in the context of Python.
I'm also using spaces both before and after the semicolon to make them stand out more.
Particularly when using intervals on more complex types the semicolon might be hard to see otherwise and makes the interval easier to recognize and read in error and debug output.
